...
import 'package:green_hornet/ui/widget/custom_bottom_nav_bar/reused_gradient.dart';
import 'package:green_hornet/ui/widget/custom_bottom_nav_bar/styles.dart';
...
typedef CustomTabBuilder = Widget Function(BuildContext context, int index, bool active);
...
class CustomConvexAppBarState extends State<CustomConvexAppBar> with TickerProviderStateMixin {
  int? _currentIndex;
  int? localIndex = 2;
  ...
  _updateAnimation({int? from, int? to, Duration duration = const Duration(milliseconds: _TRANSITION_DURATION)}) {
  ...
  Widget build(BuildContext context) {
    ...
    final lower = (2 * from + 1) / (2 * widget.count);
    final upper = (2 * to + 1) / (2 * widget.count);
    final controller = AnimationController(duration: Duration(milliseconds: 150), vsync: this);
    final curve = CurvedAnimation(
      parent: controller,
      curve: widget.curve,
    );
        ...
        Positioned.fill(
          ...
          child: Row(
            ...
            children: List.generate(widget.count, (index) {
              ...
              return Padding(
                padding: EdgeInsets.only(top: index == 2 ? 0 : 25, left: 12, right: index == 2 ? 0 : 12),
...
class StyleProvider extends InheritedWidget {
  ...
  ///  * [CustomConvexAppBar]
  ///  * [StyleHook]
  const StyleProvider({Key? key, required this.style, required Widget child}) : super(key: key, child: child);
