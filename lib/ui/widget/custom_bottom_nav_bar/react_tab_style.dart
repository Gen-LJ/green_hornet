...
class ReactTabStyle extends InnerBuilder {
  ...
  Widget build(BuildContext context, int index, bool active) {
      ...
      var children = <Widget>[
        TransitionContainer.scale(
          ...
          child: BlendImageIcon(
            ...
            color: item.blend == true ? activeColor : null,
            size: style.activeIconSize,
          ),
        ),
      ];
      if (!noLabel) {
        children.add(Text(item.title ?? '', style: style.textStyle(activeColor, item.fontFamily)));
      }
      return Container(
    ...
    var children = <Widget>[
      BlendImageIcon(
        item.icon,
        color: item.blend == true ? color : null,
        size: style.iconSize,
      ),
    ];
    if (!noLabel) {
      children.add(Text(item.title ?? '', style: style.textStyle(color, item.fontFamily)));
    }
    return Container(
      padding: const EdgeInsets.only(bottom: 2),
      child: Column(
